...
function ManageUsers() {
  ...
  // Filter users array based on search input (Name or Email matches)
  ...
  return (
    <div style={{ padding: '1.5rem', fontFamily: 'sans-serif', width: '100%', maxWidth: '1200px', margin: '0 auto', boxSizing: 'border-box' }}>
      ...
      {loading ? (
        ...
        /* FIXED: Added overflowX: 'auto' container wrapper to prevent any truncation */
        <div style={{ backgroundColor: '#fff', border: '1px solid #e2e8f0', borderRadius: '12px', overflowX: 'auto', boxShadow: '0 1px 3px rgba(0,0,0,0.05)' }}>
          {/* FIXED: Added minWidth to the table to ensure the button column has explicit spacing */}
          <table style={{ width: '100%', borderCollapse: 'collapse', textAlign: 'left', minWidth: '750px' }}>
            <thead>
              <tr style={{ backgroundColor: '#f8fafc', borderBottom: '1px solid #e2e8f0' }}>
                <th style={{ padding: '0.75rem 1rem', color: '#475569', fontWeight: '700', fontSize: '0.85rem' }}>USER IDENTIFIER</th>
                <th style={{ padding: '0.75rem 1rem', color: '#475569', fontWeight: '700', fontSize: '0.85rem' }}>EMAIL ADDRESS</th>
                <th style={{ padding: '0.75rem 1rem', color: '#475569', fontWeight: '700', fontSize: '0.85rem' }}>CLEARANCE LEVEL</th>
                {/* FIXED: Changed alignment to center for visual breathing room */}
                <th style={{ padding: '0.75rem 1rem', color: '#475569', fontWeight: '700', fontSize: '0.85rem', textAlign: 'center' }}>SECURITY ACTIONS</th>
              </tr>
            </thead>
            <tbody>
              {filteredUsers.length === 0 ? (
                ...
                filteredUsers.map((user) => (
                  <tr key={user._id} style={{ borderBottom: '1px solid #f1f5f9' }}>
                    <td style={{ padding: '0.75rem 1rem', fontWeight: '600', color: '#0f172a' }}>{user.name}</td>
                    ...
                    {/* FIXED: Updated alignment and tightened padding to keep layout uniform */}
                    <td style={{ padding: '0.75rem 1rem', textAlign: 'center' }}>
                      <button
                        ...
                        style={{
                          ...
                          whiteSpace: 'nowrap', // Prevents button text from breaking onto two lines
